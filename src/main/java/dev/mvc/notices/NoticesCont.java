...
public class NoticesCont {
  ...
  public ModelAndView list_by_search(HttpSession session,NoticesVO noticesVO) {
     ...
     * @return 페이징용으로 생성된 HTML/CSS tag 문자열
     */
    String paging = noticesProc.pagingBox(noticesVO.getNow_page(), "list_by_search.do", search_count,noticesVO.getWord());
    mav.addObject("paging", paging);
  
    mav.addObject("now_page", noticesVO.getNow_page());
  ...
  @RequestMapping(value = "/notices/list_by_search_grid.do", method = RequestMethod.GET)
  public ModelAndView list_by_search_grid(HttpSession session,NoticesVO noticesVO) {
    ModelAndView mav = new ModelAndView();
  
    // 검색 목록
    System.out.println("->grid noticesVO : "+noticesVO.toString());
    ArrayList<NoticesVO> list = this.noticesProc.list_by_search_paging(noticesVO);
    if(memberProc.isMember(session)) {
      for (int i=0;i<list.size();i++) {
        int noticesno = list.get(i).getNoticesno();
     ...
     * @return 페이징용으로 생성된 HTML/CSS tag 문자열
     */
    String paging = noticesProc.pagingBox(noticesVO.getNow_page(), "list_by_search_grid.do", search_count,noticesVO.getWord());
    mav.addObject("paging", paging);
  
    // mav.addObject("now_page", now_page);
    
    mav.setViewName("/notices/list_by_search_grid");  // /contents/list_by_cateno.jsp
